...
REGISTER_OP("RobustWarpSupGrad")
    .Input("y_true: float")    
    .Input("y_pred: float")
    .Output("grad: float");

// implement the kernel
class RobustWarpSupGradOp : public OpKernel {
 public:
  explicit RobustWarpSupGradOp(OpKernelConstruction* context) : OpKernel(context) {}
    ...
    for (int indExample = 0; indExample < batchSize; indExample++)
    {
        ...
        for (int indPos = 0; indPos < scoresPos.size(); indPos++)
        {
            if (1 - scoresPos[indPos] > 0)
                grad(indExample, labelsPos[indPos]) -= cu / batchSize;
            if (s - scoresPos[indPos] > 0)
                grad(indExample, labelsPos[indPos]) += (cu * beta) / batchSize;
        }
        
        for (int indNeg = 0; indNeg < scoresNeg.size(); indNeg++)
        {
            if (1 + scoresNeg[indNeg] > 0)
                grad(indExample, labelsNeg[indNeg]) += cu / batchSize;
            if (s + scoresNeg[indNeg] > 0)
                grad(indExample, labelsNeg[indNeg]) -= (cu * beta) / batchSize;
        }
...
// register the kernel
REGISTER_KERNEL_BUILDER(Name("RobustWarpSupGrad").Device(DEVICE_CPU), RobustWarpSupGradOp);
